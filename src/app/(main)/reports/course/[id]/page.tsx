...
export default function CourseReportPage() {
  ...
  const targetLevel = (
    (data?.course?.set_target_percentage || 0.6) * 3
  ).toFixed(2);
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <AttainmentChart
        coData={data.co_attainment}
        poData={data.po_attainment}
              ...
              <TableBody>
                {data.co_attainment?.map((co: any) => (
                  <TableRow key={co.co_id}>
                    ...
                    <TableCell>{co.attainment_level?.toFixed(2)}</TableCell>
                    <TableCell>
                      {co.attainment_level >= parseFloat(targetLevel) ? (
              ...
              <TableBody>
                {data.po_attainment?.map((po: any) => (
                  <TableRow key={po.programoutcomeid}>
                    <TableCell className="font-medium" title={po.description}>
                      {po.ponumber}
                    </TableCell>
                    <TableCell>{po.calculatedattainment?.toFixed(2)}</TableCell>
                    <TableCell>
                      {po.calculatedattainment >= parseFloat(targetLevel) ? (
                        <span className="text-green-600 font-bold">
                          Attained
                        </span>
                      ) : (
