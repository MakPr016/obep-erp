...
import { Loader2, Upload, AlertCircle, Download, FileDown } from "lucide-react"
...
export default function MarksUploadModal({ open, onClose, onSuccess, assessmentId, assignmentId }: MarksUploadModalProps) {
  ...
  const handleDownloadTemplate = () => {
    // Trigger the download via the API route
    window.location.href = `/api/assessments/cie/${assessmentId}/marks/template`;
  }
    ...
    Papa.parse(file, {
      ...
      complete: async (results) => {
        try {
          ...
          let url = `/api/assessments/cie/${assessmentId}/marks/import`;
          if (assignmentId) {
          ...
          const res = await fetch(url, {
            ...
            body: JSON.stringify({ marksData: results.data }),
          })
          ...
          if (!res.ok) {
            ...
            throw new Error(json.error || "Failed to upload marks")
          }

          toast.success(`Successfully imported marks for ${json.studentsCount} students`)
          onSuccess()
          onClose()
          setFile(null)
        } catch (err: any) {
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload student marks for this assessment.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 py-2">
          ...
          <div className="bg-muted/50 border border-dashed rounded-lg p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="text-sm text-muted-foreground text-center sm:text-left">
              ...
              <p className="text-xs">Get the correct CSV format for this test.</p>
            </div>
