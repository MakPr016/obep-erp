...
export default function AssignmentMarksUploadModal({ open, onClose, onSuccess, assignmentId }: MarksUploadModalProps) {
  ...
  const handleDownloadTemplate = () => {
    window.location.href = `/api/assignments/${assignmentId}/marks/template`
  }
    ...
    Papa.parse(file, {
      ...
      complete: async (results) => {
        try {
          ...
          const res = await fetch(`/api/assignments/${assignmentId}/marks/import`, {
            method: "POST",
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload student marks for this assignment.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 py-2">
          ...
          <div className="bg-muted/50 border border-dashed rounded-lg p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="text-sm text-muted-foreground text-center sm:text-left">
              ...
              <p className="text-xs">Get the correct CSV format with student list.</p>
            </div>
